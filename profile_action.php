...
if(isset($_POST["action"])){
  ...
  // Courier Register
  if($_POST["action"] == "register_courier"){
    ...
    // Check if username already exists.
    $sql = "SELECT * FROM tbl_couriers WHERE username = '$username'";
    $result = mysqli_query($conn, $sql);
    $checkrows = mysqli_num_rows($result);
    ...
    if($checkrows > 0) {
      ...
      $sql = "INSERT INTO tbl_couriers (fullname, 
                                      username, 
                                      email,
                                      gender,
                                      status,
  ...
  // Courier login
  if($_POST["action"] == "login_courier"){

    $username = $_POST['username'];
    $password = sha1($_POST['password']);	
  
    $sql = "SELECT * FROM tbl_couriers WHERE username = '$username' AND password = '$password'";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_row($result);
    ...
    if($row[5] == "Inactive")
    {
      ...
      if(mysqli_num_rows($result) > 0){    
        ...
        $_SESSION['email']         = $row[3];
        $_SESSION['gender']        = $row[4];
        $_SESSION['state_id']      = $row[7];
        $_SESSION['phone_no']      = $row[8];
        $_SESSION['created_date']  = $row[9];
